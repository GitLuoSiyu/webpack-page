...
(function($) {
  $.fn.slide = function(options) {
    ...
    return this.each(function() {
      ...
      var autoPlay =
        opts.autoPlay == 'false' || opts.autoPlay == false ? false : true;
      var opp = opts.opp == 'false' || opts.opp == false ? false : true;
      var autoPage =
        opts.autoPage == 'false' || opts.autoPage == false ? false : true;
      var pnLoop =
        opts.pnLoop == 'false' || opts.pnLoop == false ? false : true;
      var mouseOverStop =
        opts.mouseOverStop == 'false' || opts.mouseOverStop == false
          ? false
          : true;
      var defaultPlay =
        opts.defaultPlay == 'false' || opts.defaultPlay == false ? false : true;
      var returnDefault =
        opts.returnDefault == 'false' || opts.returnDefault == false
          ? false
          : true;
      ...
      var _tar;
      var isMarq =
        effect == 'leftMarquee' || effect == 'topMarquee' ? true : false;
            ...
            mst = setTimeout(function() {
              ...
              switch (effect) {
                case 'fade':
                  _tar
                    .stop(true, true)
                    .animate({ opacity: 'show' }, delayTime, easing, doEndFun);
                  break;
                case 'slideDown':
                  _tar
                    .stop(true, true)
                    .animate({ height: 'show' }, delayTime, easing, doEndFun);
                  break;
              }
            }, opts.triggerTime);
          },
        ...
        if (conBoxSize >= vis) {
          if (effect == 'leftLoop' || effect == 'topLoop') {
            ...
            navObjSize =
              1 +
              parseInt(
                tempS % scroll != 0 ? tempS / scroll + 1 : tempS / scroll,
              );
                ...
                .animate(
                  ...
                  function() {
                    if (__ind <= -1) {
                      conBox.css(
                        'left',
                        -(cloneNum + (navObjSize - 1) * scroll) * slideW,
                      );
                ...
                .animate(
                  ...
                  function() {
                    if (__ind <= -1) {
                      conBox.css(
                        'top',
                        -(cloneNum + (navObjSize - 1) * scroll) * slideH,
                      );
                    } else if (__ind >= navObjSize) {
...
function getPos(e) {
  ...
  l +=
    ...
    (e.currentStyle ? intval(e.currentStyle.borderLeftWidth) : 0);
  t +=
    e.offsetTop + (e.currentStyle ? intval(e.currentStyle.borderTopWidth) : 0);
